/**
In version 2 of this game, we'll require multiple keys to open the chest.
We'll also show the key inventory on the screen.
...
const NO_KEY_TEXT = "Ha Ha! You don't have the keys to open the chest."
const HAS_KEY_TEXT = "Hooray! You found the keys to open the chest."
...
async function OnBeforeProjectStart(runtime) {
    ...
    keyTwo = runtime.objects.Key2.getFirstInstance();
    keyThree = runtime.objects.Key3.getFirstInstance();
    keyInventory = runtime.objects.KeyInventory.getFirstInstance();
...
function checkForCollisions(runtime) {
    ...
    if (playerBox.testOverlap(wizard)) {
        ...
        const chestAnimation = (hasKeys) ? "open" : "closed";
        chest.setAnimation(chestAnimation);
        playerAnim.isVisible = false;
        jester.isVisible = false;
        playersText.text = (hasKeys) ? HAS_KEY_TEXT : NO_KEY_TEXT;
        playersText.isVisible = true;
    } else {
    ...
    if (key != null) {
        if (playerBox.testOverlap(key)) {
            key.destroy();
            key = null;
            keyInventoryArray[0] = 1;
        }
    }

    // Test Key2 Collision
    if (keyTwo != null) {
        if (playerBox.testOverlap(keyTwo)) {
            keyTwo.destroy();
            keyTwo = null;
            keyInventoryArray[1] = 1;
        }
    }

    // Test Key3 Collision
    if (keyThree != null) {
        if (playerBox.testOverlap(keyThree)) {
            keyThree.destroy();
            keyThree = null;
            keyInventoryArray[2] = 1;
        }
    }
}


function setInventoryAnimation() {
    const inventoryString = keyInventoryArray.map(i => i + "").join("");
    keyInventory.setAnimation(inventoryString);
    if (inventoryString === "111") {
        hasKeys = true;
    }
}